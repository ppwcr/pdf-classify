...
def vlm_ocr(img: Image.Image, model: str, processor, config, model_obj, temperature: float = 0.0, max_tokens: int = 32, timeout_s: float = 5.0):
    """
    Use VLM to extract ONLY the drawing number(s), returning a tuple:
    (best_match: str, candidates: List[str], raw_text: str)
    Includes broader patterns and a robust regex fallback.
    """
    import re, json
    sys_prompt = "Extract ONLY the construction drawing number(s)."
    user_prompt = (
        "Return STRICT JSON only: {\"drawing_no\": \"<best or empty>\", \"candidates\": [\"...\"]}.\n"
        "Examples: A1-01, S1-01, E1-01, SN1-01, A101, S201, E-101, AR000."
    )
    ...
    if isinstance(data, dict):
        best = str(data.get("drawing_no") or "").strip()
    ...
    return best, candidates, raw_text
...
def process_pdf(pdf_path: Path, dpi: int, model_id: str, debug_dir: Optional[Path], model_obj, processor, config, max_tokens: int, timeout_s: float) -> List[Dict]:
    ...
    with fitz.open(pdf_path) as doc:
        ...
        for i in tqdm(range(n), desc=f"Processing {pdf_path.name}", unit="page"):
            ...
            best, candidates, raw_text = "", [], ""
            chosen = ""; img_to_save = None
            for tag, cimg in crops:
                cimg_r = _resize_crop(cimg, max_side=max_side)
                b, cand, raw = vlm_ocr(cimg_r, model_id, processor, config, model_obj, max_tokens=max_tokens, timeout_s=timeout_s)
                if b or cand:
                    best, candidates, raw_text = b, cand, raw
                    img_to_save = cimg_r
                    chosen = tag
                    break
            if not chosen:
...
def write_csv(path: Path, rows: List[Dict]):
    path.parent.mkdir(parents=True, exist_ok=True)
    cols = ["pdf_path", "page_index", "page_width", "page_height", "drawing_no", "notes"]
